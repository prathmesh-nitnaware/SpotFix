...
import {
    ...
    ChevronRight, Camera, UploadCloud, MessageSquare, Flag, Loader2
} from 'lucide-react';
...
const StaffDashboard = () => {
    ...
    return (
        <div className="min-h-screen bg-slate-50 font-sans pb-20 w-full max-w-2xl mx-auto shadow-2xl relative">
            ...
            <nav className="bg-white border-b border-slate-200 px-4 py-3 flex items-center justify-between sticky top-0 z-50">
                ...
                <button onClick={logout} className="text-xs font-bold text-red-500 bg-red-50 px-3 py-1.5 rounded-full">Logout</button>
            </nav>

            {/* 3. PRIORITY ALERT BANNER */}
            {urgentCount > 0 && (
                <div className="bg-red-500 text-white px-4 py-3 flex items-center gap-3 shadow-inner">
                    <AlertCircle size={20} className="animate-pulse flex-shrink-0" />
                    <p className="text-xs font-bold">⚠️ <span className="font-black">{urgentCount} urgent issues</span> in your queue require immediate attention.</p>
                </div>
            )}
                                                ...
                                                <div className="pt-2 grid grid-cols-1 gap-2">
                                                    ...
                                                    {(issue.status === 'In Progress' || issue.status === 'Working' || issue.status === 'Processing') && (
                                                        <div className="space-y-3 bg-white p-4 rounded-2xl border border-slate-200">
                                                            ...
                                                            <input
                                                                type="text"
                                                                placeholder="Paste image URL of completed fix (Proof)..."
                                                                className="w-full text-xs p-2.5 bg-slate-50 rounded-lg outline-none border border-slate-200 focus:border-emerald-500 transition-colors"
                                                                value={proofImage}
                                                            ...
                                                            <button
                                                                onClick={() => handleUpdate(issue._id, { status: 'Completed', proofImage, staffNote: fieldNote }, 'complete')}
                                                                disabled={actionLoading || (!proofImage && !window.confirm("Resolve without photo proof?"))}
